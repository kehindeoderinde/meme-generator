...
class PDFIngestor(IngestorInterface):
    ...
    def parse(cls, filepath: str) -> List[QuoteModel]:
        ...
        call = subprocess.Popen(['pdftotext', filepath, tmp_filepath], stdout=subprocess.PIPE)
    ...
    def parse(cls, filepath: str) -> List[QuoteModel]:
        ...
        with open(filepath, mode='rb') as infile:
            ...
            for paragraph in document.paragraphs:
                if len(str(paragraph.text).replace('\"', '').strip('').split(' - ')) == 2:
                    body, author = str(paragraph.text).replace('\"', '').strip('').split(' - ')
                    quotes.append(QuoteModel(body, author))
