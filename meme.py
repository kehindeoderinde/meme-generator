...
from error_engine import InvalidFilePathError, InvalidTextInput
...
def generate_meme(path=None, body=None, author=None):
    ...
    if path is None:
        ...
        if not Path(path).exists():
            ...
            if not (Path(path).is_file and (Path(path).suffix == '.png' or Path(path).suffix == '.jpg')):
                raise Exception('Path specified is not a valid image. Please only use paths to .jpg or .png files')
            else:
                img = ImageModel(parent_dir=Path(path).parent, name=Path(path).name)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='CLI application to generate memes with dog pictures and quotes')
